...
export class LocalAdapter implements AdapterInterface {
     ...
     */
    async addSocket(appId: string, ws: WebSocket<AppInterface>): Promise<boolean> {
     ...
     */
    async addToChannel(appId: string, channel: string, ws: WebSocket<AppInterface>): Promise<number> {
     ...
     */
    async getSockets(appId: string, onlyLocal = false): Promise<Map<string, WebSocket<AppInterface>>> {
     ...
     */
    async getChannelSockets(appId: string, channel: string, onlyLocal = false): Promise<Map<string,WebSocket<AppInterface>>> {
    ...
    removeUser(ws: WebSocket<AppInterface>): Promise<void> {
        return this.getNamespace(ws.id).removeUser(ws);
    }

    /**
     * Get the sockets associated with an user.
     */
    getUserSockets(appId: string, userId: number|string): Promise<Set<WebSocket<AppInterface>>> {

...
export class HttpHandler {
    ...
    protected checkMessageToBroadcast(message: PusherApiMessage, app: App): Promise<PusherApiMessage> {
        return new Promise((resolve, reject) => {
            ...
            const maxEventChannels = parseInt(app.maxEventChannelsAtOnce.toString(), 10);

            if (isNaN(maxEventChannels)) {
                // Handle the case where app.maxEventChannelsAtOnce is not a valid number
                console.error("Invalid maxEventChannelsAtOnce value");
                return reject({
                    message: "Invalid configuration for maxEventChannelsAtOnce.",
                    code: 500, // Or an appropriate error code
                });
            }

            if (channels.length > maxEventChannels) {
                return reject({
                    message: `Cannot broadcast to more than ${maxEventChannels} channels at once`,
                    code: 400,
                });
            }

          
                const maxEventNameLength = parseInt(app.maxEventNameLength.toString(), 10);

                if (isNaN(maxEventNameLength)) {
                    // Handle the case where app.maxEventNameLength is not a valid number
                    console.error("Invalid maxEventNameLength value");
                    return reject({
                        message: "Invalid configuration for maxEventNameLength.",
                        code: 500, // Or an appropriate error code
                    });
                }

                if (message.name.length > maxEventNameLength) {
                    return reject({
    ...
    protected jsonBodyMiddleware(res: HttpResponse, next: CallableFunction): any {
        this.readJson(res, (body, rawBody) => {
           ...
           let requestSizeInMb = Utils.dataToMegabytes(rawBody);
            let requestLimitInMb = Number(this.server.options.httpApi.requestLimitInMb);

            if (requestSizeInMb > requestLimitInMb) {

    ...
    public options: Options = {
        ...
        queue: {
            ...
            sqs: {
                ...
                clientOptions: {},
                 consumerOptions: {
                      queueUrl: '', // Make sure to provide the correct queue URL here
                },
                queueUrl: '',
                processBatch: false,
                batchSize: 1,
        ...
        return new Promise(resolve => {
            if (this.canProcessRequests()) {
                server = server.ws(this.url('/app/:id'), {
                    ...
                    message: (ws: WebSocket<AppInterface>, message: uWebSocketMessage, isBinary: boolean) => this.wsHandler.onMessage(ws, message, isBinary),
                    open: (ws: WebSocket<AppInterface>) => this.wsHandler.onOpen(ws),
                    close: (ws: WebSocket<AppInterface>, code: number, message: uWebSocketMessage) => this.wsHandler.onClose(ws, code, message),

...
export class WsHandler {
     ...
     */
    onOpen(ws: WebSocket<AppInterface>): any {
     ...
     */
    onMessage(ws: WebSocket<AppInterface>, message: uWebSocketMessage, isBinary: boolean): any {
     ...
     */
    onClose(ws: WebSocket<AppInterface>, code: number, message: uWebSocketMessage): any {
        if (this.server.options.debug) {
     ...
     */
    evictSocketFromMemory(ws: WebSocket<AppInterface>): Promise<void> {
    ...
    async closeAllLocalSockets(): Promise<void> {
        ...
        return async.each([...namespaces], ([namespaceId, namespace]: [string, Namespace], nsCallback) => {
            namespace.getSockets().then(sockets => {
                async.each([...sockets], ([wsId, ws]: [string, WebSocket<AppInterface>], wsCallback) => {
                    try {
                        ws.sendJson({
                            event: 'pusher:error',
                            data: {
     ...
     */
    handlePong(ws: WebSocket<AppInterface>): any {
        ws.sendJson({
            event: 'pusher:pong',
            data: {},
        });
     ...
     */
    subscribeToChannel(ws: WebSocket<AppInterface>, message: PusherMessage): any {
        if (this.server.closing) {
            ws.sendJson({
                event: 'pusher:error',
                data: {
     ...
     */
    unsubscribeFromChannel(ws: WebSocket<AppInterface>, channel: string, closing = false): Promise<void> {
        let channelManager = this.getChannelManagerFor(channel);
     ...
     */
    unsubscribeFromAllChannels(ws: WebSocket<AppInterface>, closing = true): Promise<void> {
     ...
     */
    handleClientEvent(ws: WebSocket<AppInterface>, message: PusherMessage): any {
     ...
     */
    handleSignin(ws: WebSocket<AppInterface>, message: PusherMessage): void {
     ...
     */
    sendMissedCacheIfExists(ws: WebSocket<AppInterface>, channel: string) {
     ...
     */
    protected checkForValidApp(ws: WebSocket<AppInterface>): Promise<App|null> {
     ...
     */
    protected checkIfAppIsEnabled(ws: WebSocket<AppInterface>): Promise<boolean> {
     ...
     */
    protected checkAppConnectionLimit(ws: WebSocket<AppInterface>): Promise<boolean> {
     ...
     */
    signinTokenIsValid(ws: WebSocket<AppInterface>, userData: string, signatureToCheck: string): Promise<boolean> {
     ...
     */
    protected signinTokenForUserData(ws: WebSocket<AppInterface>, userData: string): Promise<string> {
     ...
     */
    protected clearTimeout(ws: WebSocket<AppInterface>): void {
        if (ws.timeout) {
     ...
     */
    protected updateTimeout(ws: WebSocket<AppInterface>): void {
        this.clearTimeout(ws);
     ...
     */
    protected setUserAuthenticationTimeout(ws: WebSocket<AppInterface>): void {

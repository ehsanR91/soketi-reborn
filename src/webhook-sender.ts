...
export class WebhookSender {
    ...
    public sendSubscriptionCount(app, channel) {
    if (!app.hasSubscriptionCountWebhooks) {
      return;
      }
      this.server.adapter.getChannelSocketsCount(app.id, channel).then(socketsCount => {
            this.send(
              app,
              {
          ...
          .catch((error) => {
            console.error("Failed to get channel sockets:", error);
          });
  }
    /**

import { AppIntegrationsConfiguration } from './../node_modules/aws-sdk/clients/qconnect.d';
...
export class Namespace {
     ...
     */
    public sockets: Map<string, WebSocket<AppInterface>> = new Map();
     ...
     */
    getSockets(): Promise<Map<string, WebSocket<AppInterface>>> {
     ...
     */
    addSocket(ws: WebSocket<AppInterface>): Promise<boolean> {
     ...
     */
    addToChannel(ws: WebSocket<AppInterface>, channel: string): Promise<number> {
        return new Promise(resolve => {
     ...
     */
    getChannelSockets(channel: string): Promise<Map<string, WebSocket<AppInterface>>> {
        return new Promise(resolve => {
            if (!this.channels.has(channel)) {
                return resolve(new Map<string, WebSocket<AppInterface>>());
            }
            ...
            resolve(
                ...
                }, new Map<string, WebSocket<AppInterface>>())
            );
        });
    }
     ...
     */
    addUser(ws: WebSocket<AppInterface>): Promise<void> {
        if (!ws.user) {
     ...
     */
    removeUser(ws: WebSocket<AppInterface>): Promise<void> {
        if (!ws.user) {
            return Promise.resolve();
        }
     ...
     */
    getUserSockets(userId: string|number): Promise<Set<WebSocket<AppInterface>>> {
        ...
        return Promise.resolve(
            [...wsIds].reduce((sockets, wsId) => {
                ...
                return sockets;
            }, new Set<WebSocket<AppInterface>>())
        );
    }
}
